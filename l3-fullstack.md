> Read the source file, you will get directions, follow the rules and complete the task

- Title: Public Utility
- Description: Essential service platforms — utility billing, school enrollment, library catalogs, transit schedules, and public health information
- Words limit: 5000 [VERY IMPORTANT]
- Think of sites like: Education bureaus, library systems

**You have to write a Write a comprehensive Product Requirements Document (~6,500 tokens). Describe user roles, auth flows, core features as user flows, data model, API design, and error handling. Be as detailed as possible — thorough PRDs produce better results.**

## What to Include:

### Write a detailed Website Description:

- What the app does and who it's for
- User roles (admin, regular user, guest…)
- How login/signup works (methods, flows, sessions)
- Main features — describe each as a step-by-step user flow
- Data model — what entities exist and how they relate
- Search, filter, sort capabilities
- Error handling, empty states, notifications
- Real-time features, payments, admin tools

### For specifically:

- Account dashboard with billing history
- Service outage map or status page
- Online payment enrollment forms
- Transit schedules, library catalog search

### Quick tips

✅ Be specific — "3-column card grid with 16px gap" not "some cards"
✅ Describe interactive flows step by step
✅ Include concrete values — hex colors, font names, pixel sizes
❌ Don't be vague — "modern UX" tells an AI nothing
❌ Don't pad with filler — quality over word count

---

Now you will write the PRD in `.md` code block. Strictly follow Words limit. Do not use any non ASCII char. Do not use bold text (ie. Don't use **Title**). List item should be started with `-`. Use table for data models.
